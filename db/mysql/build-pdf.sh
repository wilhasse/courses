...
SERIES=(innodb-architecture mysql-server-architecture)
declare -A TITLES=(
  [innodb-architecture]="InnoDB Architecture Deep-Dive"
  [mysql-server-architecture]="MySQL Server Architecture Deep-Dive"
)
...
for series in "${SERIES[@]}"; do
  ...
  sed -i -E \
    ...
    -e 's|\(\.\./([a-z-]+)/README\.md\)|(https://wilhasse.github.io/courses/\1/)|g' \
    -e 's|\(\./README\.md\)|(https://wilhasse.github.io/courses/'"$series"'/)|g' \
    "$combined"
...
PYEOF
  ...
  "$CHROME" --headless --disable-gpu --no-sandbox \
    ...
    --print-to-pdf="$SCRIPT_DIR/$series/$series.pdf" \
    "file://$work/$series.html" 2>/dev/null
  echo "   -> $series/$series.pdf"
done
