...
#include "rem0rec.h"  // if you want rec_offs_base() macros

static inline const byte*
my_rec_get_nth_field(const rec_t* rec, const ulint* offsets, ulint nth, ulint* length)
{
    ulint start = rec_offs_base(offsets)[nth];
    ulint end   = rec_offs_base(offsets)[nth + 1];

    // If flagged as SQL null:
    if ((end & REC_OFFS_SQL_NULL) != 0) {
        if (length) *length = UNIV_SQL_NULL;
        return nullptr;
    }
    start &= REC_OFFS_MASK;
    end   &= REC_OFFS_MASK;
    ...
    return reinterpret_cast<const byte*>(rec) + start;
}

/** ibrec_init_offsets_new() => fill offsets array for a COMPACT record. */
inline bool ibrec_init_offsets_new(page_t *page, rec_t* rec, table_def_t* table, ulint* offsets) {
	ulint i = 0;
	ulint offs;
	const byte* nulls;
	const byte* lens;
...
inline bool check_for_a_record(page_t *page, rec_t *rec, table_def_t *table, ulint *offsets)
{
    ...
    bool          hex_output)
{
    ...
    // where that data region begins, typically my_rec_get_nth_field(rec, offsets, 0, &len) or so.
    ...
    {
        ulint len;
        const byte *field_ptr = my_rec_get_nth_field(rec, offsets, i, &len);
...
inline bool check_constraints(rec_t *rec, table_def_t *table, ulint *offsets)
{
  int i;
  ulint len_sum = 0;
  ...
  for (i = 0; i < table->fields_count; i++)
  {
    ...
    ulint len;
    byte *field = my_rec_get_nth_field(rec, offsets, i, &len);
    if (debug)
    ...
    else
    {
      ...
      {
        len_sum += my_rec_get_nth_field_size(rec, i);
      }
    }
