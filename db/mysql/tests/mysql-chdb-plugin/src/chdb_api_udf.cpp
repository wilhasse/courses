...
void debug_log(const std::string& message) {
    ...
    if (log_file.is_open()) {
        ...
        log_file << "[" << timestamp << "] [chdb_api_udf] " << message << std::endl;
        log_file.close();
    }
}
...
bool chdb_api_query_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
    debug_log("chdb_api_query_init called with " + std::to_string(args->arg_count) + " arguments");
...
char* chdb_api_query(UDF_INIT *initid, UDF_ARGS *args, char *result,
    ...
    *error = 0;
    ...
    if (!args->args[0]) {
        debug_log("chdb_api_query called with NULL argument");
        *is_null = 1;
        return NULL;
    }
    ...
    debug_log("chdb_api_query called with: " + query);
