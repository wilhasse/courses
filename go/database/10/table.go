...
func dbUpdate(db *DB, tdef *TableDef, rec Record, mode int) (bool, error) {
	...
	values, err := getValues(tdef, rec, cols)
	if err != nil {
	...
	if req.Updated {
		...
		if err = indexOp(db, tdef, INDEX_ADD, rec); err != nil {
			return false, err // TODO: revert previous updates
		}
	}
...
func indexOp(db *DB, tdef *TableDef, op int, rec Record) error {
	for i := 1; i < len(tdef.Indexes); i++ {
		...
		assert(done) // internal consistency
	}
	return nil
}
...
func dbDelete(db *DB, tdef *TableDef, rec Record) (bool, error) {
	...
	if deleted {
		...
		indexOp(db, tdef, INDEX_DEL, Record{tdef.Cols, values})
	}
	return deleted, nil
}
